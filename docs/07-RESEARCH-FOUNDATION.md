...
**Current state:** The system is approximately 15-20% built. SerpAPI scraping, basic embeddings (`all-MiniLM-L6-v2`), a proposed SQL vector design, Gemini analysis, and many DOCX-generation scripts exist. Host inspection found SQL Server 2022, so native vector storage is not treated as implemented. The highest-value missing capabilities are hybrid lexical/semantic retrieval, cross-encoder reranking, hard eligibility gates, deduplication and diversification, a provenance-backed resume fact base, claim verification, application outcome tracking, and a closed feedback loop.
...
| Storage | SQL Server vector DDL/query examples | `RAG_JOB_SEARCH_ARCHITECTURE_REVIEW.md:118-185` | Proposal only; canonical MVP uses SQL Server 2022 plus Qdrant |
